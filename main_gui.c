...
int advisor_runner(void *data) {
    ...
    Move adv_move = mcts_search(adv_root, &advisor_arena, TIME_MID, config_advisor, NULL, NULL);
...
void render_board(SDL_Renderer *renderer) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        if (check_bit(state.white_pieces, i) || check_bit(state.white_ladies, i)) {
            ...
            if (check_bit(state.white_ladies, i)) draw_crown(renderer, cx, cy, PIECE_RADIUS);
        } else if (check_bit(state.black_pieces, i) || check_bit(state.black_ladies, i)) {
            SDL_SetRenderDrawColor(renderer, COLOR_BLACK_PIECE.r, COLOR_BLACK_PIECE.g, COLOR_BLACK_PIECE.b, 255);
            draw_circle(renderer, cx, cy, PIECE_RADIUS);
            if (check_bit(state.black_ladies, i)) draw_crown(renderer, cx, cy, PIECE_RADIUS);
        }
    }
}
...
void ai_move() {
    ...
    Move best_move = mcts_search(root, &mcts_arena, 0.05, active_config, NULL, &new_root);
...
void handle_click(int x, int y) {
    ...
    uint64_t my_pieces = (human_color == WHITE) ? (state.white_pieces | state.white_ladies) : (state.black_pieces | state.black_ladies);
