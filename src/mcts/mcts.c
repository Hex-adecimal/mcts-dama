 ...
 * 
 * Contains: mcts_create_root, mcts_search, get_tree_depth
 */
...
#include "cnn.h" // For cnn_move_to_index
#include "../params.h"
        ...
        if (config.cnn_weights) {
            // Apply NN Policy
             ...
             if (leaf->num_children == 0 && !leaf->is_terminal) {
                 // Expand logic duplication... prefer calling expand_node with policy?
                 // Current expand_node does random policy if no NN.
                 // We need to pass policy to expand_node.
                 // For now, let's assume expand_node calls cnn if passed?
                 // No, expand_node in existing code calls cnn_forward internally.
                 // We must refactor expand_node OR populate leaf->cached_policy manually here.
                 ...
                 // Manual Expansion for Async:
                  ...
                  if (legal_moves.count > 0) {
                      ...
                      for (int i=0; i<legal_moves.count; i++) {
                           ...
                           Node *child = create_node(leaf, legal_moves.moves[i], leaf->state, args->arena, config);
                           // Apply move to state... create_node does NOT apply move?
                           // checked create_node: it just inits. Caller must apply move.
                           // Simulation checks this. 
                           // Wait, standard MCTS stores state in node.
                           // We need to apply move.
...
Move mcts_search(Node *root, Arena *arena, double time_limit_seconds, MCTSConfig config,
    ...
    while (1) {
        ...
        if (elapsed >= time_limit_seconds || 
            (config.max_nodes > 0 && visits >= config.max_nodes)) {
