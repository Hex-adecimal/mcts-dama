...
void encode_state_channels(const GameState *state, float *tensor, int channel_offset) {
    for (int sq = 0; sq < 64; sq++) {
        if (check_bit(state->white_pieces, sq)) tensor[channel_offset * 64 + sq] = 1.0f;
        if (check_bit(state->white_ladies, sq)) tensor[(channel_offset + 1) * 64 + sq] = 1.0f;
        if (check_bit(state->black_pieces, sq)) tensor[(channel_offset + 2) * 64 + sq] = 1.0f;
        if (check_bit(state->black_ladies, sq)) tensor[(channel_offset + 3) * 64 + sq] = 1.0f;
    }
}

void cnn_encode_state(const GameState *state, float *tensor, float *player) {
    memset(tensor, 0, CNN_INPUT_CHANNELS * 64 * sizeof(float));
    encode_state_channels(state, tensor, 0);
    // History is missing here if only GameState is provided, should use encode_sample for training
    *player = (state->current_player == WHITE) ? 1.0f : -1.0f;
}

void cnn_encode_sample(const TrainingSample *sample, float *tensor, float *player) {
    memset(tensor, 0, CNN_INPUT_CHANNELS * 64 * sizeof(float));
    encode_state_channels(&sample->state, tensor, 0);
    // History array stores T-1 and T-2
    encode_state_channels(&sample->history[0], tensor, 4);
    encode_state_channels(&sample->history[1], tensor, 8);
    *player = (sample->state.current_player == WHITE) ? 1.0f : -1.0f;
}
