...
void cnn_forward_with_history(const CNNWeights *w, const GameState *state, 
                            ...
                            CNNOutput *out) {
    float player = (state->current_player == WHITE) ? 1.0f : -1.0f;
    float input[CNN_INPUT_CHANNELS * 64];
    memset(input, 0, sizeof(input));
    
    extern void encode_state_channels(const GameState *state, float *tensor, int channel_offset);
    encode_state_channels(state, input, 0);
    if (hist1) encode_state_channels(hist1, input, 4);
    if (hist2) encode_state_channels(hist2, input, 8);
